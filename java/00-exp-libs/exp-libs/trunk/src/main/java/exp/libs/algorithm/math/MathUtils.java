...
public class MathUtils {
	 ...
	 * <pre>
	 * 求num平方根的倒数.
	 *  此方法只有float版本�?, 对应魔鬼常数�?32位的(int) 0x5F375A86
	 *  并不存在double版本算法, 需要对�?64�?(long) 的魔鬼常�?.
	 *  
	 *  64位double可用Math库的sqrt函数求解.
	 * 
	 * ==============================================
	 * 
	 * 原算法源于卡马克�?32位浮点数的快速算�?(C++版本)�?
	 * 
	 ...
	 * 一般而言, 迭代次数越多, 精度越接近准确�?
	 * 
	 * 但是卡马克选择了一个神秘的常数 0x5f3759df 来计算那个猜测�?, 
	 * 使得算出的值非常接�?1/sqrt(n), 使得只需�?2次牛顿迭代就可以达到所需要的精度.
	 * 
	 * 
	 * 后来Lomont通过暴力测试得到了另一个稍微更好的数字: 0x5f375a86
	 * 于是有了另一个求 1/sqrt(n)的算法版�?:
	 * 
	...
	public static float toRsqrt(float n) {
		...
		i = 0x5F375A86 - (i >> 1);				// 魔鬼常数猜测�?
		n = Float.intBitsToFloat(i);			// n = *(float *)&i;
		n = n * (T_HALF - N_HALF * n * n);		// 牛顿1轮迭�?(已达到期望精�?)
//		n = n * (T_HALF - N_HALF * n * n);		// 牛顿2轮迭�?(精度更高)
		return n;
	}
	 ...
	 * 算法原理是利用异或运算的特点:任意一个数与任意一个给定的值连续异或两次，值不�?
	 * 
	 * @param a
	 * @param b
	 */
