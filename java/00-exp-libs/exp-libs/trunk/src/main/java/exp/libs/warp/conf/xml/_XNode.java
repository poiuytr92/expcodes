...
class _XNode {
	...
	/** 父节点(根节点的父节点为虚构节点) */
	private _XNode parent;
	
	/** 所有子节点集（枚举用）*/ 
	private List<_XNode> childs;
	
	/**
	 * 部分子节点坐标（精确搜索用）
	 * position(name@id) -> _XNode
	 */
	private Map<String, _XNode> childPositions;
	...
	protected _XNode(_XNode parent, Element element) {
		this.parent = parent;
		this.childs = new LinkedList<_XNode>();
		this.childPositions = new HashMap<String, _XNode>();
	...
	protected void addChild(_XNode child) {
		if(child != null) {
			childs.add(child);
	...
	protected _XNode getChild(String posORname) {
		_XNode child = childPositions.get(posORname);
		if(child == null) {
	...
	protected String getChildVal(String posORname) {
		_XNode child = getChild(posORname);
		return (child == null ? "" : child.VAL());
	}
	
	protected Iterator<_XNode> getChilds() {
