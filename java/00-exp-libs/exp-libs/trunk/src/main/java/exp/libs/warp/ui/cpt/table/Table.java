package exp.libs.warp.ui.cpt.table;
 ...
 * 用于创建表单组件，并配置其效果
 */
public class Table extends JTable implements MouseMotionListener, MouseListener {
	...
	public Table(Vector<String> header, Vector<Vector<String>> datas) {
		//调用父类方法，通过列名TableColName和数据TableData构造表单JTable
		super(new DefaultTableModel(datas, header));
		
		//设置一次只能选择表单一行
		this.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
				
		TableLength = datas.size();		//获取表单长度（表格行数）
	...
	public void mouseMoved(MouseEvent e) {
		...
		for(int r = 0; r < TableLength; r++) {
			if(r == currentMouseRow){
				Table.this.setBackground(Color.green);
				break;
			}
		}
