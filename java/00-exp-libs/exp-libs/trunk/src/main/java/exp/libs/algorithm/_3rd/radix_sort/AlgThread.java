package exp.libs.algorithm._3rd.radix_sort;
/* AlgThread.java */
...
/*-*/frame.waitStep();
	...
	/*-*/frame.setText(1, "Processing digit " + (j+1) + "...");
		    ...
		    /*-*/frame.Highlight(17);
                    ...
                    int destBinNum = getDigit( (Integer) sourceBin[i].get(k), j);/*-*/frame.Highlight(19);
                    destBin[ destBinNum ].put( (Integer) sourceBin[i].get(k) );/*-*/frame.Highlight(20);
	...
	/*-*/ frame.setText(0, "Extracting number from bin.."); frame.setText(2, "");
        for (int j = 0; j < lastBin.length; j++) {/*-*/frame.Highlight(33);
            for (int k = 0; k < lastBin[j].size(); k++) {/*-*/frame.Highlight(34);
                a[i++] = (Integer) lastBin[j].get(k);/*-*/frame.Highlight(35);
