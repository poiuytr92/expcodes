...
public class JsonUtils {
	...
	/**
	 * 从json对象中取指定键名的string类型�?
	 * @param json json对象
	 * @param key 键名
	 * @return string类型�?, 若无值则返回""
	 */
	public static String getStr(JSONObject json, String key) {
		...
		try {
			...
			log.error("从JSON中提�? string 类型�? [{}] 失败.", key, e);
		}
		return val;
	}
	...
	public static String getStr(JSONObject json, String key, String defavlt) {
		...
		try {
			...
			log.error("从JSON中提�? string 类型�? [{}] 失败.", key);
		}
		return val;
	}
	
	/**
	 * 从json对象中取指定键名的bool类型�?.
	 * @param json json对象
	 * @param key 键名
	 * @param defavlt 默认�?
	 * @return bool类型�?, 若无值则返回默认�?
	 */
	public static boolean getBool(JSONObject json, String key, boolean defavlt) {
		...
		try {
			...
			log.error("从JSON中提�? bool 类型�? [{}] 失败.", key, e);
		}
		return val;
	}
	...
	public static int getInt(JSONObject json, String key, int defavlt) {
		...
		try {
			...
			log.error("从JSON中提�? int 类型�? [{}] 失败.", key, e);
		}
		return val;
	}
	
	/**
	 * 从json对象中取指定键名的long类型�?.
	 * @param json json对象
	 * @param key 键名
	 * @param defavlt 默认�?
	 * @return long类型�?, 若无值则返回默认�?
	 */
	public static long getLong(JSONObject json, String key, long defavlt) {
		...
		try {
			...
			log.error("从JSON中提�? long 类型�? [{}] 失败.", key, e);
		}
		return val;
	}
	...
	public static JSONObject getObject(JSONObject json, String key) {
		...
		try {
			...
			val = new JSONObject();
			log.error("从JSON中提�? object 对象 [{}] 失败.", key, e);
		}
		return val;
	}
	...
	public static JSONArray getArray(JSONObject json, String key) {
		...
		try {
			...
			val = new JSONArray();
			log.error("从JSON中提�? array 对象 [{}] 失败.", key, e);
		}
		return val;
	}
	 ...
	 * 	<PRE>
	 * 		String[]数组.
	 * 		由于第三方构件问�?, 若JSONArray无元�?, 会自动填充一�? "" 元素, 
	 * 		此时返回的不�? String[0], 而是 String[1] { "" }
	 * 	</PRE>
	 */
	 ...
	 * 		String[]数组.
	 * 		由于第三方构件问�?, 若JSONArray无元�?, 会自动填充一�? "" 元素, 
	 * 		此时返回的不�? List<String>(0), 而是 List<String>(1) { "" }
	 * 	</PRE>
	 */
