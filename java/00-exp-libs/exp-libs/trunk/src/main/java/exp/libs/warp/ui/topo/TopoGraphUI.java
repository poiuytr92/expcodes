...
public class TopoGraphUI extends PopChildWindow {
	...
	/** 建议最大的节点数（超过这个数量时GEF演算速度会极�?, 导致陷入长时间无法生成拓扑图的假死状态） */
	...
	protected void setComponentsLayout(JPanel root) {
		...
		JPanel tips = new JPanel(new FlowLayout()); {
			...
			JLabel normalTips = new JLabel("[ �? : 普通节�? ]");
	...
	private void paintGraph(TopoGraph topoGraph) {
		...
		List<GraphEdge> viewEgdes = !useGEF ? 
				calculatePositionByPolar(topoGraph) : 	// 使用极坐标反算直角坐�?
	...
	private List<GraphEdge> calculatePositionByPolar(TopoGraph topoGraph) {
		int size = topoGraph.nodeSize();// 拓扑图规�?
		Node src = topoGraph.getSrc();	// 拓扑图源�?
		
		GraphNode cellEdgeSrc = new GraphNode(src.getAliasName(), 0, 0);// 绘制图原�?
		List<GraphEdge> viewEdges = new LinkedList<GraphEdge>();	// 绘制图边�?
		Map<String, GraphNode> viewNodeMap = new HashMap<String, GraphNode>(); { // 绘制图节点集
			viewNodeMap.put(src.getName(), cellEdgeSrc);
			cellEdgeSrc.markGraphSrc();	// 标记为绘制源�?
		}
		...
		for(int idx = 0; idx < size; idx++) {
			...
			int split = (idx == 0 ? edgeSnks.size() : edgeSnks.size() - 1);	// 父节点不算在�?
	 ...
	 * 	那么直接等分这两个节点的圆周角时，很容易使得某些邻接点在绘图时重�?.
	 * 
	...
	private double[] _subMultipleAngle(int split) {
		...
		final int ROTATION = RandomUtils.genInt(90);	// 90度以内的随机旋转�?
	...
	/**
	 * 以src作为极坐标系原点，利用snk相对于极轴的旋转角计算snk在直角坐标系的相对位�?
	 * 
	 * @param viewEdgeSrc 极坐标系原点
	 * @param viewEdgeSnk 目标�?
	 * @param weight src与snk的边权（长度�?
	 * @param theta snk相对于极轴在极坐标系中的旋转�?
	 ...
	 * 	(绘制图的原点在绘制面板左上角�? 向右为X正向，向下为Y正向, 即与真正的直角坐标系相对于原点上下反�?)
	 * @param viewNodeMap 绘制节点�?
	 */
	...
	private List<GraphEdge> calculatePositionByGEF(TopoGraph topoGraph) {
		...
		Map<String, GraphNode> viewNodeMap = // 唯一性点集，避免重复放入同一节点到GEF造成拓扑图离�?
	...
	private Map<DefaultGraphCell, Object> getNodeAttribute(GraphNode node) {
		...
		if(useGEF == true) {
			...
			bound = new Rectangle2D.Double(
					...
					node.getWidth(), node.getHeight());	// 强制设定所呈现节点的宽�?
		} else {
