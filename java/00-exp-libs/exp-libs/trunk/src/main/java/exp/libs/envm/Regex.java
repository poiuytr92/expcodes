...
public enum Regex {
	...
	REAL_POSITIVE("(\\+?([1-9]|[1-9]\\d+)(\\.\\d+)?)", "正实�?"),
	
	REAL_NEGATIVE("(-([1-9]|[1-9]\\d+)(\\.\\d+)?)", "负实�?"),
	
	REAL_NOT_NEGATIVE("(\\+?(\\d|[1-9]\\d+)(\\.\\d+)?)", "非负实数"),
	
	INTEGER("([+-]?(\\d|[1-9]\\d+))", "整数"),
	
	INTEGER_POSITIVE("(\\+?([1-9]|[1-9]\\d+))", "正整�?"),
	
	INTEGER_NEGATIVE("(-([1-9]|[1-9]\\d+))", "负整�?"),
	...
	FLOAT_NOT_NEGATIVE("(\\+?(\\d|[1-9]\\d+)(\\.\\d+)?)", "非负浮点�?"),
	...
	USERNAME("([a-zA-Z]\\w{5,})", "用户�?: 字母开�?, 至少6�?, 字母+数字+下划�?"),
	
	PASSWORD("([\\w\\-+`=\\[\\];',./~!@#$%^&*()\\|{}:\"<>\\?\\\\]{6,})", "密码: 至少6�?, 字母+数字+符号"),
	...
	SOCKET(StrUtils.concat("(", IPV4, ":", PORT, ")"), "套接�?"), 
	
	MD5(StrUtils.concat("(([0-9|A-F|a-f]{16}){1,2})"), "MD5�?"), 
