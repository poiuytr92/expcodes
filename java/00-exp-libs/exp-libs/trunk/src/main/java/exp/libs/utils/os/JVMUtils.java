...
public final class JVMUtils {
	...
	private static long _fullSizeOf(Object obj) {
		Map<Object, Object> visited = new IdentityHashMap<Object, Object>();	//已访问列表，比较key时是�? [引用相等] 为标�?, 而非[对象相等]
		Stack<Object> stack = new Stack<Object>();	// 登记对象的内部对象的�?(只要对象嵌套层次不深、单个对象数组长度不大，栈可保证不会在计算大量对象时导致内存溢出)
		stack.push(obj);
	...
	private static void collectInternalObject(Class<?> clazz, Object obj, Stack<Object> stack) {
		while (clazz != null) {
				...
				if (!Modifier.isStatic(field.getModifiers())) {
					...
					if (field.getType().isPrimitive()) {
						...
						try {
							...
							if (o != null) {
								stack.add(o);	// 将成员对象放入栈中，一遍弹出后继续检�?
							}
			...
			clazz = clazz.getSuperclass();// 找父类class，直到没有父�?
		}
	}
