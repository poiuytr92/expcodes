 ...
 * 表单组件
 */
public class TableComponents extends JTable implements MouseMotionListener, MouseListener {
	...
	public TableComponents(Vector<Vector<Object>> TableData, Vector<String> TableColName) {
		...
		public void adjustTableColumnWidths(JTable table) {
			...
			for (int col = 0; col < tcm.getColumnCount(); col++) {	//循环处理每一列
				...
				for (int row = 0; row < rowCount; row++) {			//循环处理第i列的每一行，用单元格渲染器计算第col列第row行的单元格长度
					int CellWidth = (int) table.getCellRenderer(row, col).getTableCellRendererComponent(table, table.getValueAt(row, col), false, false, row, col).getPreferredSize().getWidth();	//Exp:廖权斌
	...
	public void mouseMoved(MouseEvent e) {
		...
		for(int r = 0; r < TableLength; r++) {
			if(r == currentMouseRow){
				TableComponents.this.setBackground(Color.green);
				break;
			}
		}
