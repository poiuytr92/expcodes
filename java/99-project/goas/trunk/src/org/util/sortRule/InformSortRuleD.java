...
//通告记录根据id值降序排序
public class InformSortRuleD implements Comparator {
		...
		if(ymd1[0].equals(ymd2[0])) {
			if(ymd1[1].equals(ymd2[1])) {
				if(ymd1[2].equals(ymd2[2])) {
					return ((Inform)o2).getId().intValue() - ((Inform)o1).getId().intValue();
				}
				else {
					return Integer.parseInt(ymd2[2]) - Integer.parseInt(ymd1[2]);
				}
			}
			else {
				return Integer.parseInt(ymd2[1]) - Integer.parseInt(ymd1[1]);
			}
		}
		else {
			return Integer.parseInt(ymd2[0]) - Integer.parseInt(ymd1[0]);
		}
	}
