...
public class EncryptUtils {
	 ...
	 * 通过 skey 计算GTK�?.
	 * ---------------------------------
	 * 先用 外置的JS算法 计算 GTK�? 当使�? JS计算失败 时，才使用内置算法计算�?
	 * 外置JS算法主要是为了在QQ更新了GTK算法情况下，可以对应灵活修改�?
	 * 
	 * @param pskey 从登陆cookie中提取的标识(每次登陆时随机生�?)
	 * @return GTK�?
	 */
	 ...
	 * =====================================
	 * 	类似于GTK, 这个 QzoneToken 也是在每次登陆时自动生成的一个固定�?, 但是生成算法相对复杂（需要jother解码�?, 
	 *  因此此处取巧, 直接在页面源码中提取QzoneToken�?
	 * 
	 * @param homePageSource QQ空间首页的页面源�?
	 * @return QzoneToken
	 */
