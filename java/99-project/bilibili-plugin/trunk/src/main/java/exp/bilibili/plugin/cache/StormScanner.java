...
//		this.scanCookie = Browser.COOKIES();
		...
		this.scan = false;
		this.lives = new LinkedList<String>();
	}
...
//			loginUI._view();
	...
	public boolean reflashLives() {
		Map<String, String> header = toGetHeadParams(scanCookie, "all");
		...
		for(int page = 1; page <= PAGE_LIMIT; page++) {
			...
			lives.addAll(getRoomIds(response));
		}
		client.close();
		
		log.info("已更新 [Top {}] 的人气直播间.", lives.size());
	 ...
	 * @return
	 */
	private List<String> getRoomIds(String response) {
	...
	public void checkStorm() {
		...
		for(String roomId : lives) {
			requests.put("roomid", roomId);
			Map<String, String> headers = toGetHeadParams(scanCookie, roomId);
	 ...
	 * @return
	 */
	private List<String> getRaffleIds(String roomId, String response) {
	...
	private void joinStorm(String roomId, List<String> raffleIds) {
		if(raffleIds.size() <= 0) {
			return;
		}
	...
	private static Map<String, String> toGetHeadParams(String cookies, String roomId) {
		...
		params.put(HttpUtils.HEAD.KEY.REFERER, LIVE_URL.concat(roomId));
		return params;
	}
