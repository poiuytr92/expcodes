...
//				}
	 ...
	 * 获取节奏风暴的抽奖ID
	 * @param roomId
	 * @param response {"code":0,"msg":"","message":"","data":{"id":318289831272,"roomid":291623,"num":100,"send_num":"1","time":76,"content":"要优雅，不要�?","hasJoin":0,"storm_gif":"https://static.hdslb.com/live-static/live-room/images/gift-section/mobilegift/2/jiezou.gif?2017011901"}}
	 * @return
	 */
	...
	public static boolean toLottery(int roomId, String raffleId) {
		...
		while(isExist && !cookies.isEmpty()) {	// 理论上节奏风暴可以在完结前一直抢到成功为�?
			isExist = false;	// 避免cookies队列没人参与陷入死循�?
			
			Iterator<BiliCookie> cookieIts = cookies.iterator();
			while(cookieIts.hasNext()) {
				BiliCookie cookie = cookieIts.next();
				
				// 未绑定手机或未实名认证的账号无法参与节奏风暴  (FIXME 未实名也可参�?, 但是需要填验证�?, 目前未能自动识别验证�?)
				...
				if(StrUtils.isEmpty(reason)) {
					log.info("[{}] 参与直播�? [{}] 抽奖成功(节奏风暴)", cookie.NICKNAME(), roomId);
					cookieIts.remove();	// 已经成功抽奖的在本轮无需再抽
					isExist = true;
					cnt++;
					
				} else if(reason.contains("访问被拒�?")) {
					UIUtils.statistics("失败(", reason, "): 账号 [", cookie.NICKNAME(), "]");
					cookieIts.remove();	// 被临时封禁抽奖的账号无需再抽
					isExist = true;
					
				} else {
					log.info("[{}] 参与直播�? [{}] 抽奖失败(节奏风暴)", cookie.NICKNAME(), roomId);
		...
		if(cnt > 0) {
			UIUtils.statistics("成功(节奏风暴x", cnt, "): 直播�? [", roomId, "]");
