...
/*
	题意分析：
	 证明任意一个大于4的偶数n都可以写成两个奇素数（非2质数）之和, 其中n∈[6,1000000)
	 其实就是证明100万以内的哥德巴赫猜想.
...
int main(void) {
	bool prime[LEN];		// 素数集, 标记每个自然数是否为素数
	findPrimes(prime);		// 找出范围内所有素数

	int n = 0;
	while(cin >> n && n > 0) {
		printGoldbach(prime, n);	// 打印哥德巴赫猜想的结果
	}
	return 0;
}
...
void findPrimes(bool* prime) {
	memset(prime, true, sizeof(bool) * LEN);	// 注意memset是按字节覆写内存的
